...
public interface CarDao {
    ...
    @Select("select carstock from carStock where carId = #{carId}")
    Integer getStock(String carId);

    @Update("update carstock set carStock=carStock-1 where carId = #{carId}")
    void updateStock(String carId);
}
